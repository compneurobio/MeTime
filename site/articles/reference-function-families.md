...
- Wrappers: For easier data manipulation we have wrappers for mutate,
  select, rename and fiter from the dplyr package
- QC helpers: There are additional mod\_ functions that can help user to
  remove features/samples based on missingness or on variance (Example:
  `mod_filter_samples_by_missingness`)

### `calc_*`

- Purpose: perform analysis methods.
- Typical usage: one result object per named analysis.
- These set of analysis are currently available, a more detailed
  description can be found in choosing methods:
  ...
  - Regression (Linear models, Linear mixed models and Generalized
...
- Functions include:
  ...
  - meta_conservation (compare conservation index analysis across two
    results)
  - meta_feature_overlap (compare feature selection analysis across two
    results)
  - meta_network_overlap (compare two networks)
  - meta_matrix_similarity (compare two results from
    calc_distance_samples or calc_correlation_metabolites)
