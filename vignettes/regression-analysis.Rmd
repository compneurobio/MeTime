...
This vignette demonstrates several regression workflows, including meta-analysis, time-interaction, GAMM, and baseline linear models.
...
adni_nmr_data <- adni_nmr_data %>%
  ...
  mod_filter(
    which_data=which_data,
    type="row_data",
    !DXGrp_long %in% c("mix_CN_stable", "mix_MCI_stable", "mix_AD_stable", "MCI_CN_revert")
  ) %>%
  mod_mutate(
    which_data="phenotype_data",
    type="data",
  ...
  mod_merge_data_and_row_data(
    which_data=which_data,
    cols_list=list(data=NULL, row_data=c("APOEGrp", "PTGENDER", "Age", "BMI", "PTEDUCAT", "DXGrp_long")),
    name="nmr_data_merged"
  ) %>%
  mod_merge_data(
    which_data=c("phenotype_data", "medication_data", "nmr_data_merged"),
    filter_sample="biocrates_data",
    name="regression",
    append=TRUE,
    cols_list=list(
      nmr_data=names(object@list_of_data[["nmr_data_merged"]]),
      phenotype_data=c(
        "CSF_Roche_Abeta42", "LEntCtx_vol", "LEntCtx_Thick", "REntCtx_vol", "REntCtx_Thick",
  ...
  mod_mutate(
    ...
    "entorhinal_thickness"=(LEntCtx_Thick + REntCtx_Thick) %>% as.numeric() %>% scale(),
    "entorhinal_volume"=(LEntCtx_vol + REntCtx_vol) %>% as.numeric() %>% scale(),
    "ptau"=CSF_Roche_PTau %>% as.numeric() %>% log2() %>% scale(),
    "tau"=CSF_Roche_Tau %>% as.numeric() %>% log2() %>% scale(),
    "tau_by_abeta42"=(as.numeric(CSF_Roche_Tau) / as.numeric(CSF_Roche_Abeta42)) %>% log2() %>% scale(),
    "adas_cog13"=ADAS13 %>% sqrt() %>% scale(),
    "memory_score"=ADNI_MEM %>% scale(),
...
std_meta <- adni_nmr_data %>%
  add_data(which_data="regression", type="col_data", x=meta, id="id") %>%
  calc_lmm(
    which_data="regression",
    name="adni_nmr_meta",
    stratifications=NULL,
    cols_for_meta=list(c(sub_pathway="Group")),
    num_cores=12
  ) %>%
  mod_generate_plots(type="regression", interactive=TRUE)

# Time-interaction analysis.
std_lmm <- adni_nmr_data %>%
  add_data(which_data="regression", type="col_data", x=lmm, id="id") %>%
  calc_lmm(
    which_data="regression",
...
std_lm0 <- adni_nmr_data %>%
  add_data(which_data="regression", type="col_data", x=lm_model, id="id") %>%
  calc_lm(
    which_data="regression",
    name="biocrates_lm0",
    stratifications=NULL,
