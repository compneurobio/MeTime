...
#  MeTime::mod_filter(id %in% setdiff(my_analyzer@list_of_col_data[[medication_name]]$id,exclude_med), which_data=medication_name,type="c
#ol_data") %>% # filter any medications that should be excluded
#  MeTime::calc_featureselection_boruta(which_x=medication_name, # here we use medication but you can use any other dataset
...
#saveRDS(selected_full,
#        file=paste0(path_out,"metabolon_medsel_all.rds"))
