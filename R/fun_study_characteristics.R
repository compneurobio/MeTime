...
#' @export
setGeneric("distribution_plotter", function(object, colname, which_data) standardGeneric("distribution_plotter") )

setMethod("distribution_plotter", "metab_analyser",function(object, colname, which_data) {
	...
	if(var_type %in% "bar") {
		...
		density_plot <- ggplot(plot_data, aes_string(x=colname, fill="Timepoints")) + geom_density(alpha=0.3) + 
					geom_vline(data=mu, aes(xintercept=mean, color=Timepoints), linetype="dashed") +
 					scale_fill_manual(values=palette_timepoints) + theme_minimal()
