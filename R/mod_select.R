...
#' @description Modification (mod) function to select columns in data, row_data or col_data. It is a wrapper function to dplyr::select()
...
#' @seealso [mod_mutate], [mod_rename], [mod_filter]
...
#' will be returned
#' @export
setGeneric("mod_select", function(object, which_data, type="data", ...) standardGeneric("mod_select"))
setMethod("mod_select", "metime_analyser", function(object, which_data, type="data", ...) {
	if(length(which_data)!=1) {
        warning("mod_select(): length of which_data is not 1, exiting without making any changes")
        return(object)
    }
    if(!type %in% c("data", "row_data", "col_data", "results")) {
        warning("mod_select(): type is unknown. Please check the accepted values and exiting without making any changes")
        return(object)
    }
    filter_exprs <- enquos(...)
    ...
    if(type %in% "data") {
        ...
        data <- data %>% dplyr::select(!!!filter_exprs)
        object@list_of_data[[which_data]] <- data
        #adjust col data accordingly
        object@list_of_col_data[[which_data]] <- object %>% 
        get_coldata(which_data=which_data) %>%  
        dplyr::filter(id %in% colnames(object@list_of_data[[which_data]]))  
        ...
        results$plot_data <- lapply(seq_along(results$plot_data), function(x) {
            results$plot_data[[x]] <- results$plot_data[[x]] %>% dplyr::select(!!!filter_exprs)
            return(results$plot_data[[x]])
        })
      return(results$plot_data)
    }
    
    object <- object %>% add_function_info(function_name="mod_select", 
