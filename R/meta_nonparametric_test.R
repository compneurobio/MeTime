#' Function to perform non-parametric tests on two sets of results or results of interest
#' @description Meta function to perform non-parametric tests on different types of results to perform meta analysis of interest
#' @param object An S4 object of class metime_analyser
#' @param results_index A character/numeric input for a results. The length should be equal to 1.
#' @param type character input to define the type of test. Two types are available: "kruskal", "wilcox"
#' @param formula formula(class) input for the test functions. See rstatix::wilcox_test() or rstatix::kruskal_test()
#' @param ... additional arguments for either rstatix::wilcox_test() or rstatix::kruskal_test() based on input
#' @param name character input to set the name of the results
#' @seealso [meta_parametric_test] [rstatix::wilcox_test] [rstatix::kruskal_test]
#' @returns An S4 object of class metime_analyser with this meta analysis stored as results
#' @export
setGeneric("meta_nonparametric_test", function(object, results_index, type, formula, name="meta_nonparametric_test_1", ...) standardGeneric("meta_nonparametric_test"))
setMethod("meta_nonparametric_test", "metime_analyser", function(object, results_index, type, formula, name="meta_nonparametric_test_1", ...) {
		if(!class(formula) %in% "formula") {
			warning("Formula should be of class formula. Exiting without making any changes.")
		}
		if(!type %in% c("kruskal", "wilcox")) {
		...
		if(type %in% "kruskal") {
			plot_data <- lapply(seq_along(plot_data), function(ind) {
					test_res <- rstatix::kruskal_test(plot_data[[ind]], formula=formula, ...)
					return(test_res)
				})
		} else if(type %in% "wilcox") {
			plot_data <- lapply(seq_along(plot_data), function(ind) {
					test_res <- rstatix::wilcox_test(plot_data[[ind]], formula=formula, ...)
					return(test_res)
				})
		}
		...
		object <- object %>% 
				...
				add_function_info(function_name="meta_nonparametric_test", 
