...
#' @param stratifications Character vector with colnames that are to be used for stratification. Not a list.
...
#' @export
setGeneric("viz_distribution_plotter", function(object, colname, which_data, stratifications, phenotype) standardGeneric("viz_distribution_plotter"))
setMethod("viz_distribution_plotter", "metime_analyser",function(object, colname, which_data, stratifications=NULL, phenotype) {
	...
	if(var_type %in% "bar") {
		...
		timepoints <- rownames(phenotype) %>% gsub(pattern="[a-z|A-Z][0-9]+_", replacement="")
		levels <- timepoints %>% gsub(pattern="[a-z|A-Z]", replacement="") %>% as.numeric() %>% sort()
		...
		line_plot <- ggplot(plot_data, aes_string(x="Timepoints", y="Frequency", group=colname)) + 
					 geom_line(aes_string(color=colname)) + geom_point(aes_string(color=colname)) + theme_classic()
		...
		if(is.null(stratifications)) {
			...
			strats_df <- phenotype[ ,stratifications]
			...
			density_plot <- ggplot(plot_data, aes_string(x=colname, fill="Timepoints")) + geom_density(alpha=0.3) + 
								geom_vline(data=mu, aes(xintercept=mean, color=Timepoints), linetype="dashed") +
			 					scale_fill_manual(values=palette_timepoints) + theme_classic() + facet_wrap(stratifications)
		}
