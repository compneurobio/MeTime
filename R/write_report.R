...
setMethod("write_report", "metime_analyser", function(object, title=NULL, file=NULL, write_results=F, device="html", interactive=F, author=NULL) {
  ...
  for(nr_analyzer in seq_along(analyzer_list)) {
    results_list[[nr_analyzer]] <- lapply(seq_along(analyzer_list[[nr_analyzer]]@results), function(nr_results) {
      results <- analyzer_list[[nr_analyzer]]@results[[nr_results]]
      list(
        functions_applied = results$functions_applied,
        items = lapply(seq_along(results$plot_data), function(nr_data) {
          list(
            h2 = names(results$plot_data)[nr_data],
            plot_data = results$plot_data[[nr_data]],
            calc_type = results$information$calc_type[nr_data],
            calc_info = results$information$calc_info[nr_data],
  ...
  out_header <- write_report_header(
    title=ifelse(is.null(title), paste0("metime", Sys.Date()), tools::file_path_sans_ext(file)),
    type=device
  )
